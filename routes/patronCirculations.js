...
module.exports = app => {
  ...
  app.get('/:libraryId/patrons/:option', function (req, res) {
      ...
      switch(req.params.option) {
        ...
        case 'new': query = `SELECT libraryId, month, year, SUM(PatronCount) as count FROM NewPatrons WHERE (MONTH > month(NOW()) AND YEAR = YEAR(NOW()) - 1 OR YEAR = YEAR(NOW())) and libraryID = ${req.params.libraryId} GROUP BY YEAR, MONTH ORDER BY YEAR DESC, MONTH DESC`; break;
      }
